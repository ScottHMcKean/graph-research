...
def test_optimizer_convergence():
    df = pd.DataFrame({"value": [10, 15, 20], "target": [12, 18, 22]})
...
def test_optimizer_max_iterations():
    ...
    optimizer = DataFrameOptimizer(
        max_iterations=max_iterations, convergence_threshold=0.01
    )

    # Run optimization
    result = optimizer.workflow.invoke({"df": df, "iteration": 0, "converged": False})
